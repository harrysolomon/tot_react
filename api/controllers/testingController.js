exports.testing = (req, res, next) => {
    ...
    final_result["out_of_pocket"].reduce( (prev, curr,i) =>  final_result["cum_out_of_pocket"][i] =  prev + curr, 0)

    final_result["cum_loan"] = []
    final_result["loan"].reduce( (prev, curr,i) =>  final_result["cum_loan"][i] = prev + curr, 0)

    final_result["cum_s1"] = []
    final_result["salary_1"].reduce( (prev, curr,i) =>  final_result["cum_s1"][i] = prev + curr, 0)

    final_result["cum_s2"] = []
    final_result["salary_2"].reduce( (prev, curr,i) =>  final_result["cum_s2"][i] = prev + curr, 0)
    ...
    for(let i = 0; i < final_result["cum_opportunity_cash"].length; i++){
     ...
     for(let i = 0; i < final_result["cum_out_of_pocket"].length; i++){
        final_result["total_cost"].push(final_result["cum_out_of_pocket"][i] + final_result["cum_loan"][i]);
     }

     for(let i = 0; i < final_result["total_cost"].length; i++){
        final_result["s1_income"].push(final_result["cum_s1"][i] - final_result["total_cost"][i]);
     }

     for(let i = 0; i < final_result["total_opportunity_cash"].length; i++){
        final_result["s2_income"].push(final_result["cum_s2"][i] + final_result["total_opportunity_cash"][i]);
     }
